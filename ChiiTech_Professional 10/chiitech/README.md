...
## Latest round: three real bugs found by an actual end-to-end test, and fixed

A full signup → login → RLS → auditor → soft-delete → restore test (run directly
against the database, inside a transaction that was rolled back afterward, so it
never touched real data) surfaced three genuine bugs that no earlier check — schema
review, the security advisor, RLS policy reading — would have caught, because none
of those exercise actual query privileges or runtime function execution:

1. **No table had any base privileges granted to `authenticated`.** RLS decides
   *which* rows a query can touch; Postgres separately requires the base
   `GRANT SELECT/INSERT/UPDATE/DELETE` before it even evaluates RLS. Every table
   here was created via raw SQL rather than Supabase's own table editor (which
   issues these grants automatically), so that grant never existed — meaning
   every logged-in user's every query was blocked outright. This was the actual
   cause of "login and stuff not working": auth itself succeeded, and the
   dashboard's first real data query failed immediately after. Fixed in migration 16.
2. **Three tables had silently ended up in the wrong schema** (`app_private`
   instead of `public`) — not through any tracked migration; moved back in
   migration 15 with their RLS policies, indexes, and triggers intact.
3. **Two functions had real logic bugs**, not just permissions: the audit-log hash
   trigger couldn't find Postgres's `digest()` function (it lives in Supabase's
   `extensions` schema, not `public` — migration 17), and `auditor_login()` had a
   column name that was ambiguous with its own return type, meaning auditor
   sign-in could never actually have succeeded (migration 18).

All four fixes are now in `supabase/migrations/`, verified byte-for-byte the same
way as every other migration in that folder. The full 21-step test passes clean
after them.

## Earlier round: account deletion, disaster recovery, audit materiality/findings, a React island
